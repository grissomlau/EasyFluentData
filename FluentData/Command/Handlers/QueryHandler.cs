    ...
    internal class QueryHandler<TEntity>
    {
        ...
        internal TList ExecuteMany<TList>(Action<TEntity, IDataReader> customMapperReader,
            ...
            if (_typeHandler.IterateDataReader) {
                ...
                //-- Begin added by Dhensk on 2013-09-26 10:37  Desc: To suppose multiple table query
                while (_typeHandler is QueryDataTableHandler<TEntity> && !_data.Reader.IsClosed) {
                    var item2 = (TEntity)_typeHandler.HandleType(customMapperReader, customMapperDynamic);
                    items.Add(item2);
                }
                //-- End added
            }
