        ...
        public string GetSqlForSelectBuilder(SelectBuilderData data) {
            ...
            if (data.PagingCurrentPage == 1) {
                if (data.PagingItemsPerPage == 0)
                    sql.AppendFormat("select {0}", data.Distinct);
                else
                    sql.AppendFormat("select {0} top {1}", data.Distinct, data.PagingItemsPerPage.ToString());
                ...
                var pagedSql = string.Format(@"with PagedPersons as
								...
								where fluentdata_RowNumber between {3} and {4}",
                                             data.Select,
                    //data.OrderBy, -- dh 2015-02-09
                                             data.RowNumberOrderBy,
                                             sql,
                                             data.GetFromItems(),
                                             data.GetToItems());
                return pagedSql;
